...
The first test program in this subdirectory [./msg_sock_hdlr_test_01.c](./msg_sock_hdlr_test_01.c) also includes tests of boundary-case circumstances (handling of unterminated input strings). Note that this first test is intended to be used as two processes simultaneously (for example, in separate cmd windows) with one process designated 'receive' (started first and running) and then the other designated 'send' - see comments in code or usage messages appearing when program input parameters are missing.
